...
const FeaturedProducts = () => {
  ...
  useEffect(() => {
    ...
    .then((data) => {
      if(data.length > 0){
        const product = data.filter((item) => item.productCategory === "featured");
        setDatas(product)
      }
    })
  },[])
      ...
      <div className="my-8 xl:my-10">
        ...
        <div className="container mx-auto px-3 sm:px-4 md:px-5 lg:px-7 xl:px-10">
          ...
          <div className="mb-5 text-center">
            <h2 className="text-2xl sm:text-2xl md:text-xl lg:text-3xl xl:text-4xl 2xl:text-4xl font-medium">Featured Product</h2>
          </div>
          {/* title end */}
          {/* cart start */}
